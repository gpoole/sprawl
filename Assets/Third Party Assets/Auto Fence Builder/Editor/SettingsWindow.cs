...
public class SettingsWindow : EditorWindow {
	...
	void OnGUI() {
		...
		string[] subModeNames = {"Use Single Box Collider", "Keep Original Colliders (Custom Objects Only)", "No Colliders"};
		int[] subModeNums = {0,1,2};
		EditorGUI.BeginChangeCheck();
		afb.railColliderMode = EditorGUILayout.IntPopup("Rail Colliders: ", afb.railColliderMode, subModeNames, subModeNums);
		if(EditorGUI.EndChangeCheck() ){
			isDirty = true;
		}
		EditorGUI.BeginChangeCheck();
		...
		if (GUILayout.Button("OK")) {
			...
			if(afb.railColliderMode < 2 || afb.postColliderMode < 2 || afb.extraColliderMode < 2){
		...
		if (GUILayout.Button("Cancel")) {
			...
			afb.railColliderMode = tempRailColliderMode;

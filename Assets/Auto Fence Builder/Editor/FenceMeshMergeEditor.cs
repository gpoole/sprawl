...
public class FenceMeshMergeEditor : Editor {
	...
	/*GameObject CombineMeshes(List<GameObject> allGameObjects, string name){
		...
		for(int i=0; i< allGameObjects.Count; i++){
			...
			GameObject go, parentGO = allGameObjects[i].gameObject;
			List<GameObject> allGameObjectsInParent = GetAllMeshGameObjectsFromGameObject(parentGO);
			List<Mesh> allMeshesInParent = GetAllMeshesFromGameObject(parentGO);

			for(int m=0; m<allMeshesInParent.Count; m++)
			{
				go = allGameObjectsInParent[m];
				defaultMat = go.GetComponent<Renderer>().sharedMaterial;

				Mesh mesh = (Mesh) Instantiate( allMeshesInParent[m] );
				...
				mesh.vertices = newVerts;
				combiners[i].mesh = mesh;
				Transform finalTrans = Instantiate(parentGO.transform) as Transform;
				combiners[i].transform = finalTrans.localToWorldMatrix;
				DestroyImmediate(finalTrans.gameObject);
			}
